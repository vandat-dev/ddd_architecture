...
class TokenService:
    ...
    def refresh_access_token(self, refresh_token: str, user: User) -> Union[str, None]:
        """
        Generate a new access token using a valid refresh token.

        Args:
            refresh_token (str): The refresh token to validate.
            user (User): User model instance for the new access token.

        Returns:
            str | None: New access token if refresh token is valid, None otherwise.
        """
        refresh_payload = self.validate_token(refresh_token)
        if refresh_payload and refresh_payload.get("sub") == str(user.id):
